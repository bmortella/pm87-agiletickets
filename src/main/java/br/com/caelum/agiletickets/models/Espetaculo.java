...
public class Espetaculo {
	...
	public List<Sessao> criaSessoes(LocalDate inicio, LocalDate fim,
		...
		int quantidadeDeSessoes = 0;

		if (periodicidade.equals(Periodicidade.SEMANAL)) {
			quantidadeDeSessoes = Weeks.weeksBetween(inicio, fim).getWeeks();
		} else if (periodicidade.equals(Periodicidade.DIARIA)) {
			quantidadeDeSessoes = Days.daysBetween(inicio, fim).getDays();
		}

		for (int i = 0; i <= quantidadeDeSessoes; i++) {
			Sessao sessao = new Sessao();
			sessao.setEspetaculo(this);
			LocalDate dataDia = inicio.plusWeeks(i);
			sessao.setInicio(dataDia.toDateTime(horario));
